<html>
	...
	<body>
		...
		<?php
			function drawTable($numberOfRows,$numberOfCols,$stid) {
				...
				while ($row = oci_fetch_row($stid)) {
					...
					echo "</td>";
						...
						echo "<td>";
							echo "<a href='edit.php?key=".$row[0]."' method=post>Edit</a>";
						echo "</td>";
						echo "<td>";
							echo "<a href='delete.php?key=".$row[0]."' method=post>Delete</a>";
						echo "</td>";
					echo "</tr>";
				}
				echo "</table>";
            ...
            include 'conn.php';
			$error = '';
			$statement = "select * from MAIN_GAMES_VIEW where UPPER(\"Game Title\") LIKE UPPER('%' || :title_b || '%')";
			...
			if ($numberOfRows==0) {
				echo "<section class='container'>";
				echo "<h1>Ain't no thing but a chicken wing</h1>";
				echo "</section>";
			} else {
				if (oci_execute($stid)) {
